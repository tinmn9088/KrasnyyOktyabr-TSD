...
Функция ОбработатьШтрихкодРоссыпи(Знач Задание, Знач Комплектовщик, Знач Штрихкод) Экспорт
    ...
    Попытка
        ...
        Если Выборка.Следующий() Тогда
            ...
            // 6.1. Уменьшение количества у задания на отбор
        ...
        // 7. Дополнение документа на внетреннее перемещение
        //@skip-check reading-attribute-from-database
        Если ЗначениеЗаполнено(Задание.ДокументНаПеремещение) Тогда

            ДокументНаПеремещение = Задание.ДокументНаПеремещение.ПолучитьОбъект();
            ДокументНаПеремещение.ЗаписатьКодМаркировки(КодМаркировки);
        ...
        // 8. Зафиксировать транзакцию, если код возврата указывает на отсутствие ошибок
        Если ОписаниеОбработкиШтрихкода.КодВозврата = КодВозвратаУспех() Тогда
...
Функция ОбработатьШтрихкодКороба(Знач Задание, Знач Комплектовщик, Знач Штрихкод) Экспорт
    ...
    Попытка
        ...
        Если Выборка.Следующий() Тогда
            ...
            // 4.1. Проверка на то, что после уменьшения количества оно не станет отрицательным
            Если ВычестьКоличество <= ПредыдущееКоличество Тогда
                
                // 4.1.1. Уменьшение количества у задания на отбор
        ...
        // 5. Дополнение документа на внетреннее перемещение
        //@skip-check reading-attribute-from-database
        Если ЗначениеЗаполнено(Задание.ДокументНаПеремещение) Тогда

            ДокументНаПеремещение = Задание.ДокументНаПеремещение.ПолучитьОбъект();
            ДокументНаПеремещение.ЗаписатьНомерКороба(НомерКороба);
        ...
        // 6. Зафиксировать транзакцию, если код возврата указывает на отсутствие ошибок
        Если ОписаниеОбработкиШтрихкода.КодВозврата = КодВозвратаУспех() Тогда
