...
Процедура ПриОткрытии(Отказ)
    ...
    Если Объект.ПроверкаЗавершена Тогда
...
Асинх Процедура СписокНоменклатурВыборАсинх(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
    ...
    Если Не Объект.ИспользуетсяМаркировкаТоваров Тогда
        ...
        GTIN = GTINЧерезНомерНоменклатуры(Объект.Ссылка, ВыбраннаяСтрока);

        НовоеКоличествоПринятогоТовара = Ждать ВвестиЧислоАсинх(ПредыдущееКоличествоПринятогоТовара, GTIN, , 3);

        Если НовоеКоличествоПринятогоТовара <> Неопределено И НовоеКоличествоПринятогоТовара > 0
            И НовоеКоличествоПринятогоТовара <> ПредыдущееКоличествоПринятогоТовара Тогда
...
Процедура ОбработатьШтрихкод(Штрихкод)
    ...
    Перем ОписаниеОбработкиШтрихкода; // См. Приходы.ОбработатьКодМаркировки
    ...
    Иначе
        ...
        Если Не ЗначениеЗаполнено(GTIN) Тогда
            ...
            ПоказатьВажноеОповещениеОбОшибкеСканированияШтрихкода("Штрихкод без GTIN", Штрихкод);

            Возврат;

        КонецЕсли;

        ОписаниеОбработкиШтрихкода = ОбработатьGTIN(Объект.Ссылка, GTIN);
...
// См. Приходы.ОбработатьКодМаркировки
&НаСервереБезКонтекста
Функция ОбработатьКодМаркировки(Знач АктПриемкиТоваров, Знач КодМаркировки)

    Возврат Приходы.ОбработатьКодМаркировки(АктПриемкиТоваров, КодМаркировки);

КонецФункции

// См. Приходы.ОбработатьGTIN
//@skip-check method-param-value-type
&НаСервереБезКонтекста
Функция ОбработатьGTIN(Знач АктПриемкиТоваров, Знач GTIN)

    //@skip-check invocation-parameter-type-intersect
    Возврат Приходы.ОбработатьGTIN(АктПриемкиТоваров, GTIN);
...
//  Строка 
&НаСервереБезКонтекста
Функция GTINЧерезНомерНоменклатуры(Знач АктПриемкиТоваров, Знач НомерНоменклатуры)
...
// Процедура воспроизводит звуковой сигнал, соответствующий ошибке сканирования и
...
//  Пояснение - Строка
&НаКлиенте
Процедура ПоказатьВажноеОповещениеОбОшибкеСканированияШтрихкода(Знач Заголовок, Знач Пояснение)
...
// См. Приходы.ОсталосьПроверить
&НаСервереБезКонтекста
Функция ОсталосьПроверить(Знач АктПриемкиТоваров)

    Возврат Приходы.ОсталосьПроверить(АктПриемкиТоваров);
