...
Процедура ОбработатьШтрихкод(Штрихкод)
    ...
    Если ФормаПроцесс = ПредопределенноеЗначение("Перечисление.ПроцессНаПроизводстве.ПечатьЯрлыков")
        И СтрДлина(Штрихкод) <> 20 Тогда

        ПоказатьВажноеОповещениеОбОшибке("Некорректная длина", Штрихкод);
        Возврат;
    ...
    Если Режим = "Удаление" Тогда
        ...
        Если Не Дубликат(ФормаСотрудник, ФормаПроцесс, ШтрихкодИзм) Тогда
        ...
        Результат = УдалитьШтрихкод(ФормаСотрудник, ФормаПроцесс, ШтрихкодИзм);
...
&НаСервереБезКонтекста
Функция Дубликат(Знач Сотрудник, Знач Процесс, Знач Штрихкод)

    Возврат Производство.НайтиШтрихкод(Сотрудник, Процесс, Штрихкод);
