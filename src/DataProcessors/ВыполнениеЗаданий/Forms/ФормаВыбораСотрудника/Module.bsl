...
Процедура ОткрытьВыборЗадания(Знач ТабельныйНомер)
    ...
    ИначеЕсли ТипЗнч(Сотрудник) = Тип("СправочникСсылка.СотрудникиНаПроизводстве") Тогда

        ПараметрыФормы = Новый Структура("Сотрудник", Сотрудник);
        ОткрытьФорму("Обработка.ВыполнениеЗаданий.Форма.ФормаВыбораПроцессаНаПроизводстве", ПараметрыФормы);

    КонецЕсли;

КонецПроцедуры

// Функция выполняет поиск сотрудника по табельному номеру.
// 
// Параметры:
//  ТабельныйНомер - Строка
// 
// Возвращаемое значение:
//  Неопределено, СправочникСсылка.Комплектовщики, СправочникСсылка.СотрудникиСклада, СправочникСсылка.СотрудникиНаПроизводстве - Сотрудник с нужным табельным номером
&НаСервере
Функция НайтиСотрудникаПоТабельномуНомеру(Знач ТабельныйНомер)
    ...
    Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
                          ...
                          |
                          |ВЫБРАТЬ ПЕРВЫЕ 1
                          |	СотрудникиНаПроизводстве.Ссылка
                          |ИЗ
                          |	Справочник.СотрудникиНаПроизводстве КАК СотрудникиНаПроизводстве
                          |ГДЕ
                          |	СотрудникиНаПроизводстве.ТабельныйНомер = &ТабельныйНомер");
    ...
    Сотрудник = Выборка.Сотрудник; // СправочникСсылка.Комплектовщики, СправочникСсылка.СотрудникиСклада, СправочникСсылка.СотрудникиНаПроизводстве
