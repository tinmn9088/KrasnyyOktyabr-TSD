...
Процедура ОбработатьШтрихкод(Штрихкод)
    ...
    ИначеЕсли Тип = ПредопределенноеЗначение("Перечисление.ТипШтрихкода.НомерЯчейки") Тогда
        ...
        ПоказатьОповещение("Обработка номера ячейки", Штрихкод);
        ...
        ПоказатьВажноеОповещениеОбУспешномВыполнении("Номер ячейки изменен", Штрихкод);
...
// Процедура воспроизводит звуковой сигнал, соответствующий ошибке, и 
// показывает важное оповещение об ошибке.
&НаКлиенте
Процедура ПоказатьОповещениеОбОшибкеНевыбраннойЯчейки()

    ПоказатьВажноеОповещениеОбОшибке("Ячейка не выбрана", "Сначала отсканируйте номер ячейки");
