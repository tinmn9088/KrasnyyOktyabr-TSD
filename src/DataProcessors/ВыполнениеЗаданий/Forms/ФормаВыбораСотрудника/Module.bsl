...
#КонецЕсли
    ...
    ИначеЕсли ТипЗнч(Сотрудник) = Тип("СправочникСсылка.СотрудникиСклада") Тогда
        ...
        ПараметрыФормы = Новый Структура("СотрудникСклада", Сотрудник);
...
// 
// Возвращаемое значение:
//  СправочникСсылка.Комплектовщики, СправочникСсылка.СотрудникиСклада - Сотрудник с нужным табельным номером или пустая ссылка
&НаСервере
Функция НайтиСотрудникаПоТабельномуНомеру(Знач ТабельныйНомер)

    // 1. Поиск среди комлпектовщиков
    Сотрудник = Справочники.Комплектовщики.НайтиПоТабельномуНомеру(ТабельныйНомер); // СправочникСсылка.Комплектовщики, СправочникСсылка.СотрудникиСклада
    
    // 2. Поиск среди сотрудников склада, если не удалось найти комплектовщика
    Если Не ЗначениеЗаполнено(Сотрудник) Тогда

        Сотрудник = Справочники.СотрудникиСклада.НайтиПоТабельномуНомеру(ТабельныйНомер);
