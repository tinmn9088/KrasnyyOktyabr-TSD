...
#Если МобильныйКлиент Или МобильноеПриложениеКлиент Тогда
	...
	// Отображение используется ли маркировка товаров
    Элементы.ИспользуетсяМаркировкаТоваров.Видимость = Ложь;

    Если Объект.ИспользуетсяМаркировкаТоваров Тогда

        Элементы.Документ.Рамка = Новый Рамка(ТипРамкиЭлементаУправления.ДвойноеПодчеркивание, 2);
...
Асинх Процедура СписокНоменклатурВыборАсинх(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
    ...
    Если Не Объект.ИспользуетсяМаркировкаТоваров Тогда
        ...
        НомерНоменклатуры = ВыбраннаяСтрока.НомерНоменклатуры; // Число
		
        //@skip-check property-return-type
        ПредыдущееКоличествоПринятогоТовара = ВыбраннаяСтрока.КоличествоПринятогоТовара; // Число

        НовоеКоличествоПринятогоТовара = Ждать ВвестиЧислоАсинх(ПредыдущееКоличествоПринятогоТовара, "Принято", , 0);

        ОбновитьКоличествоПринятогоТовара(Объект.Ссылка, НомерНоменклатуры, НовоеКоличествоПринятогоТовара);
