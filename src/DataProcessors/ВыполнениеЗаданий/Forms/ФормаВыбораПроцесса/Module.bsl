...
Процедура ПриОткрытии(Отказ)
    ...
    Если Не ТекущийПользовательИмеетПравоВыполнятьРеализацию() Тогда

        СделатьКнопкуНедоступной(Элементы.КнопкаВыбратьРеализацию);
...
&НаКлиенте
Процедура ВыбратьПроцессРеализацию(Команда)
...
#КонецЕсли
    ...
    // Открытие формы выбора 
    ПараметрыФормы = Новый Структура("Сотрудник", ФормаСотрудник);
    ОткрытьФорму("Обработка.ВыполнениеРеализации.Форма.ФормаВыбораРеализации", ПараметрыФормы);
...
&НаСервере
Функция ТекущийПользовательИмеетПравоВыполнятьРеализацию()

    Возврат ПравоДоступа("Использование", Метаданные.Обработки.ВыполнениеРеализации);
